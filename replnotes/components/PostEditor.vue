<template>
    <div>
        ...
        <text-area-input
            ...
            @input="$emit('input')"
            @change="
                (val) => $emit('change', { key: 'description', value: val })
            "
        ></text-area-input>
        <multi-choice
            id="tags"
...
import { readFile, parseThumbnailsFromNbJson, downloadNotebook } from '~/services/notebook'
            ...
            if (newValue && typeof newValue !== Blob) {
                ...
                readFile(newValue)
                    .then((text) => {
                        ...
                        this.thumbnails = parseThumbnailsFromNbJson(
                            this.content
                        )
                    })
                    .catch((error) => {
                        ...
                        this.$emit(
                            'error',
                            'Only Jupyter Notebooks are supported'
                        )
                    })
            }
        },
