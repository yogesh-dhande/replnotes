<template>
  <div>
    ...
    <modal :show="showModal" @close="closeModal">
      <div
        class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-16 sm:align-middle sm:max-w-4xl sm:w-full"
        role="dialog"
        aria-modal="true"
        ...
        <window>
          ...
          <template v-slot:body>
            <div class="bg-gray-700 px-4 py-4 sm:p-6 sm:pb-4">
              <div
                class="grid grid-cols-1 mt-4 px-4 sm:grid-cols-2 md:grid-cols-3 gap-4"
              >
                <button
                  class="border border-gray-500 rounded shadow hover:shadow-2xl text-gray-300"
                  @click="() => select(null)"
                >
                  No Thumbnail
                </button>
